<h2> Trivia! </h2>

			<p>
				1. What do you often do when you're home?
			</p>

			<p id="q1">
				ANSWER
			</p>

			<button type="button" onclick="document.getElementById('q1').innerHTML = 'Video Games, Mix Music, Draw'">Click to see answer</button>
			
			<p>
				2. What country do you want to visit the most?
			</p>

			<p id="q2">
				ANSWER
			</p>

			<button type="button" onclick="document.getElementById('q2').innerHTML = 'JAPAN'">Click to see answer</button>

			<p>
				3. What other professions do you want?
			</p>

			<p id="q3">
				ANSWER
			</p>

			<button type="button" onclick="document.getElementById('q3').innerHTML = 'Professional DJ, Character Designer, Game Analyst, Professional Basketball player'">Click to see answer</button>

			<p>
				4. What do you find most satisfying?
			</p>

			<p id="q4">
				ANSWER
			</p>

			<button type="button" onclick="document.getElementById('q4').innerHTML = 'Winning'">Click to see answer</button>

			<p>
				5. Best Trait?
			</p>

			<p id="q5">
				ANSWER
			</p>

			<button type="button" onclick="document.getElementById('q5').innerHTML = 'Chill'">Click to see answer</button>
			
				<p>
				1. Favorate Gaming company?
			</p>

			<p id="q1">
				ANSWER
			</p>

			<button type="button" onclick="document.getElementById('q1').innerHTML = 'Square Enix'">Click to see answer</button>
			
			<p>
				2. What games do you usually play?
			</p>

			<p id="q2">
				ANSWER
			</p>

			<button type="button" onclick="document.getElementById('q2').innerHTML = 'RPG/Moba or Shooting games'">Click to see answer</button>

			<p>
				3. Favorate Basketball shoes brand
			</p>

			<p id="q3">
				ANSWER
			</p>

			<button type="button" onclick="document.getElementById('q3').innerHTML = 'Nike'">Click to see answer</button>

			<p>
				4. What do you find most satisfying?
			</p>

			<p id="q4">
				ANSWER
			</p>

			<button type="button" onclick="document.getElementById('q4').innerHTML = 'Winning'">Click to see answer</button>

			<p>
				5. Best Trait?
			</p>

			<p id="q5">
				ANSWER
			</p>

			<button type="button" onclick="document.getElementById('q5').innerHTML = 'Chill'">Click to see answer</button>


			